...
public class ExemploCursos {
	...
	public static void main(String[] args) {
		...
		System.out.println("*-----------------------------------------*");
		System.out.println("Cursos com numero de alunos maior ou igual a 100");

		cursos.stream().filter(curso -> curso.getAlunos() >= 100).forEach(c -> System.out.println(c.getNome()));

		System.out.println("*-----------------------------------------*");
		System.out.println("Numero de alunos nesses cursos");
		...
		System.out.println(cursos.stream().filter(curso -> curso.getAlunos() >= 100).mapToInt(Curso::getAlunos).sum());
		...
		cursos.stream().filter(c -> c.getAlunos() <= 50).collect(Collectors.toMap(c -> c.getNome(), c -> c.getAlunos()))
				.forEach(
						(nome, alunos) -> System.out.println(String.format("Curso de %s tem %s alunos", nome, alunos)));

		System.out.println("*-----------------------------------------*");

		Double mediaDeAlunos = cursos.stream().mapToInt(c -> c.getAlunos()).average().getAsDouble();

		System.out.println("Media de alunos por cursos: " + mediaDeAlunos);
		
		Stream<Curso> stream = cursos.stream()
				   .filter(c -> c.getAlunos() > 50);
		
		List<Curso> filtrados = stream.collect(Collectors.toList());
